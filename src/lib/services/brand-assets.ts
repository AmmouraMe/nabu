 ...
 */
export async function getBrandTextById(db: D1Database, textId: string): Promise<BrandText | null> {
	const row = await db
 ...
 */
export async function getBrandTexts(db: D1Database, brandProfileId: string): Promise<BrandText[]> {
 ...
 * Delete a text asset.
 */
export async function deleteBrandText(db: D1Database, textId: string): Promise<void> {
	await db.prepare('DELETE FROM brand_texts WHERE id = ?').bind(textId).run();
}
...
export async function getBrandMedia(
	...
	return (result.results || []).map((row) => mapRowToMedia(row as Record<string, unknown>));
}
...
export async function updateBrandMedia(
	...
	if (params.r2Key !== undefined) {
		sets.push('r2_key = ?');
		values.push(params.r2Key);
	}
 ...
 */
export async function deleteBrandMedia(db: D1Database, mediaId: string): Promise<void> {
	// Delete variants first (cascade should handle this, but be explicit)
	await db.prepare('DELETE FROM brand_media_variants WHERE brand_media_id = ?').bind(mediaId).run();

	await db.prepare('DELETE FROM brand_media WHERE id = ?').bind(mediaId).run();
}
...
export async function getMediaVariants(
	...
	return (result.results || []).map((row) => mapRowToVariant(row as Record<string, unknown>));
}

/**
 * Delete a specific variant.
 */
export async function deleteMediaVariant(db: D1Database, variantId: string): Promise<void> {
	await db.prepare('DELETE FROM brand_media_variants WHERE id = ?').bind(variantId).run();
}
...
export async function getBrandAssetSummary(
	...
	const imageRow = await db
		.prepare(
			"SELECT COUNT(*) as count FROM brand_media WHERE brand_profile_id = ? AND media_type = 'image'"
		)
		.bind(brandProfileId)
		.first<{ count: number }>();
	...
	const audioRow = await db
		.prepare(
			"SELECT COUNT(*) as count FROM brand_media WHERE brand_profile_id = ? AND media_type = 'audio'"
		)
		.bind(brandProfileId)
		.first<{ count: number }>();
	...
	const videoRow = await db
		.prepare(
			"SELECT COUNT(*) as count FROM brand_media WHERE brand_profile_id = ? AND media_type = 'video'"
		)
		.bind(brandProfileId)
		.first<{ count: number }>();
