...
function starsHtml(): string {
	return STAR_POSITIONS.map(
		(s) => `<span style="top:${s.top};left:${s.left};width:${s.size}px;height:${s.size}px;"></span>`
	).join('\n    ');
}
...
function buildVoiceCards(p: BrandBookProfile): VoiceCard[] {
	...
	const isProfessional =
		tone.includes('professional') || tone.includes('authoritative') || tone.includes('formal');
	const isPlayful =
	...
	const isWarm =
		...
		tone.includes('empathetic') ||
		tone.includes('caring');
...
function dividerPage(number: string, title: string, tagline: string, items: string[]): string {
