...
describe('extractVideoUrl', () => {
	...
	it('extracts videoUrl from part', () => {
		const data = {
			candidates: [{ content: { parts: [{ videoUrl: 'https://storage.googleapis.com/vid.mp4' }] } }]
		};
	...
	it('extracts from operation response wrapper', () => {
		const data = {
			...
			response: {
				candidates: [{ content: { parts: [{ videoUrl: 'https://example.com/result.mp4' }] } }]
			}
		};
...
describe('generateVideoScript', () => {
	...
	it('returns title, script, and durationSeconds', async () => {
		const ai = makeAi({
			...
			script: 'A developer types at keyboard',
			durationSeconds: 5
		});
		const result = await generateVideoScript(
			ai as Parameters<typeof generateVideoScript>[0],
			brand,
			'developer tools'
		);
